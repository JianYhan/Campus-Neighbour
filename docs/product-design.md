...
状态：已有可运行实现，验证见[第二轮记录](implementation/iteration-2.md)。下文未被更新的规则继续作为设计目标；涉及需求说明书D01—D13的决定仍待确认。
...
桌面：顶部显示Logo、商品入口、季节专区、发布、个人入口、语言和主题切换，页尾可访问消息／交易／管理。手机：底部为逛一逛、消息、发布、交易、我的五项；筛选在列表上方折行展示。断点720px，主要表单与详情改单列，商品仍双列。聊天桌面左列表右内容，手机在会话列表和独立聊天页间切换。两端同一组件保持草稿，尺寸变化不重新初始化表单。未来若采用抽屉筛选，应保留现有筛选状态。
