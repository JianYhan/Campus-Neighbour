...
版本：v0.1 分工讨论稿｜日期：2026年9月23日

用途：让每位成员知道负责什么、需要学什么、先看什么、什么时候开始、把结果交给谁，以及怎样判断任务完成。

本手册细化[开发协作与任务计划](../CONTRIBUTING.md)，属于八份工程文档中的协作配套材料，不增加新的业务模块。岗位编号用于讨论，姓名、负责人确认记录和内部任务日期均待填写。本文件描述计划，不代表已经分配岗位、完成活动或通过测试。

**阅读导航：** 认领岗位先看[九人总览](#2-九个岗位总览)和[首轮任务](#8-首轮两至三天的具体任务)；了解接力顺序看[完整流程](#6-从项目启动到最终提交的完整顺序)和[单功能示例](#7-一个功能怎样从一个人交到下一个人)；实际开工查[岗位详情](#4-每个岗位的详细任务)、[记录模板](#10-任务交接和审核怎么记录)及[最终交付](#13-课程节点与最终交付)。

## 1 先统一分工方式

建议由项目发起人承担主要开发与技术整合，另设一名开发协作者，其余成员承担需求、交互、测试、运行、文档与课程交付。每个人对具体成果负责，并参与相关决定。
...
| --- | --- | --- | --- | --- |
| R1 | 技术负责人／核心开发 | 核心交易流程可运行，架构和整合可维护 | 熟悉前后端及数据库 | 待填写 |
| R2 | 开发协作者 | 独立小功能、测试和代码复核 | 有编程基础，或与 R1 配对学习 | 待填写 |
| R3 | 需求负责人 | 业务规则明确、范围有依据、验收有标准 | 能梳理流程、提问和记录结论 | 待填写 |
| R4 | 交互与体验负责人 | 电脑和手机页面可理解、可操作 | 愿意学习原型与页面设计 | 待填写 |
| R5 | 功能测试负责人 | 用户操作经过独立验证，问题可复现 | 细致，能按步骤执行和比较结果 | 待填写 |
| R6 | 自动化与质量负责人 | 关键规则、权限和异常有可重复测试 | 有编程基础，愿意学习测试工具 | 待填写 |
| R7 | 运行与交付负责人 | 他人可以启动、部署和恢复项目 | 愿意学习环境配置、容器和日志 | 待填写 |
| R8 | 技术文档负责人 | 报告与手册准确、完整、来源清楚 | 能整理材料并理解系统流程 | 待填写 |
| R9 | 项目协调与展示负责人 | 任务有人接、课程材料准时交、展示可复现 | 沟通、跟进和表达能力 | 待填写 |

R1 建议由项目发起人担任，仍由小组填写确认；其余岗位先表达意愿，再通过首轮小任务调整。不要将“不熟悉代码”固定成整个学期不能参与开发的标签。
...
**首次任务：** 组织岗位认领与首轮任务确认；优先核对9月25日申报书中的待填信息、组员姓名和学号。
...
```mermaid
flowchart TD
    A[全体认领岗位 R9记录] --> B[R1 R7 R8核对已有成果和版本]
    B --> C[R3澄清本轮需求]
    C --> D[R4交互设计]
    C --> E[R1 R2技术设计]
    C --> F[R5 R6准备验收与测试场景]
...
| --- | --- | --- | --- |
| S0 启动与现状核对 | R9组织全体；R1／R7／R8查看已有文档、分支和证据 | 记录认领岗位、基线版本、已完成及待验证项；交全体 | 每人有首项任务，明确以哪个版本为准；未确认状态保留待验证 |
...
| R9 | 收集岗位意愿、创建首轮任务记录，核对申报书待填项 | 全体 | 每项有负责人和审核人；提交信息由成员确认 |
...
先由 R9 组织大家阅读第2节，认领岗位意向；再按第8节领取一项小任务，填写负责人、审核人和内部日期。首轮结束后根据成果、兴趣和工作量调整长期分工。
